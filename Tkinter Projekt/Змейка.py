...
def make_mouse():
    ...
    canvas.delete(mouse)
    mouse = canvas.create_rectangle(x_mouse, y_mouse, x_mouse + 50, y_mouse + 50, fill='green')
...
def crash():
    ...
    for i in range(0, len(listpos) - 1):
        if (x_snake + 25) in range(listpos[i][0], listpos[i][0] + 51) and (y_snake + 25) in range(listpos[i][1],
                                                                                                  listpos[i][1] + 51):
            canvas.delete("all")
            img = ImageTk.PhotoImage(Image.open("1.jpg").resize((1536, 864)))
